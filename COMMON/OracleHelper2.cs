...
namespace mymeswpf.COMMON
{
    public sealed class OracleHelper2
    {
        ...
        private static int ExecuteNonQuery(string connectionString, CommandType commandType, string commandText, params OracleParameter[] commandParameters)
        {
        ...
        /// <param name="commandType">内容类型</param>
        /// <param name="parameterValues">可变对象参数组</param>
        /// <returns>返回影响数据表中的行数。</returns>

        public static int ExecuteNonQuery(string connectionString, string spName, CommandType commandType, params object[] parameterValues)
        {
            ...
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                return ExecuteNonQuery(connectionString, commandType, spName, commandParameters);
            }
            //otherwise we can just call the SP without params
            else
            {
                return ExecuteNonQuery(connectionString, commandType, spName);
            }
        }
        ...
        private static int ExecuteNonQuery(OracleConnection connection, CommandType commandType, string commandText, params OracleParameter[] commandParameters)
        {
        ...
        private static DataSet ExecuteDataset(string connectionString, CommandType commandType, string commandText, params OracleParameter[] commandParameters)
        {
            //create & open a OracleConnection, and dispose of it after we are done.
            using (OracleConnection cn = new OracleConnection(connectionString))
            {
        ...
        public static DataSet ExecuteDataset(string connectionString, string spName, CommandType commandType, params object[] parameterValues)
        {
            //if we receive parameter values, we need to figure out where they go
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                //call the overload that takes an array of OracleParameters
                return ExecuteDataset(connectionString, commandType, spName, commandParameters);
            }
            //otherwise we can just call the SP without params
            else
            {
                return ExecuteDataset(connectionString, commandType, spName);
            }
        }
        ...
        private static object ExecuteScalar(string connectionString, CommandType commandType, string commandText, params OracleParameter[] commandParameters)
        {
            //create & open a OracleConnection, and dispose of it after we are done.
            using (OracleConnection cn = new OracleConnection(connectionString))
            {
        ...
        /// <summary>
        /// 可用于事务操作。操作成功后返回操作第一个数据的数据类型。可无参数操作！
        /// </summary>
        /// <param name="connectionString">连接字符串</param>
        /// <param name="spName">储存过程名称</param>
        /// <param name="commandType">内容类型</param>
        ...
        public static object ExecuteScalar(string connectionString, string spName, CommandType commandType, params object[] parameterValues)
        {
            //if we receive parameter values, we need to figure out where they go
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                //call the overload that takes an array of OracleParameters
                return ExecuteScalar(connectionString, commandType, spName, commandParameters);
            }
            //otherwise we can just call the SP without params
            else
            {
                return ExecuteScalar(connectionString, commandType, spName);
            }
        }
